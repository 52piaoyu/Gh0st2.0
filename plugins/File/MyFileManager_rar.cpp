...
//#pragma comment(lib,"shwlapi.lib")
...
#ifdef _UNICODE
	SE LxShellExecute = (SE)GetProcAddress(shell32, "ShellExecuteW");
#else
	SE LxShellExecute = (SE)GetProcAddress(shell32, "ShellExecuteA");
#endif

	LxShellExecute(GetDesktopWindow(),_T("open"),lpFile,NULL,NULL,nShowCmd);
...
UINT CMyFileManager::SendDriveList()
{
	...
	BYTE	DriveList[1024];
	DWORD dwOffset;
	char	FileSystem[MAX_PATH],*pDrive = NULL;
...
void CMyFileManager::PackFile(LPBYTE lpBuffer)
{
	...
	ZeroMemory(lpRarPath,1024*sizeof(TCHAR));
	ZeroMemory(lpRealPath,1024*sizeof(TCHAR));
	
	ReadRegEx(HKEY_CLASSES_ROOT, 
						 _T("WinRAR\\shell\\open\\command"),
						 _T(""),
						 REG_SZ, 
						 lpRarPath, 
						 NULL, 
