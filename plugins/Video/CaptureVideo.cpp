...
HRESULT CCaptureVideo::Open(int iDeviceID,int iPress)
{
	...
	if( FAILED( hr ) ){
		return hr;
			 ...
			 mCB.lWidth = vih->bmiHeader.biWidth;   //得到设备的高度和宽度，构造BMPINFO时能用到
			 ...
			 hr = m_pGrabber->SetCallback( &mCB, 1 );      //这里的mCB是友元类一会我们要讲他
			 //设置视频捕捉窗口
