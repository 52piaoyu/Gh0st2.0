...
void CIOCPServer::OnAccept()
{
	...
	if (setsockopt(pContext->m_Socket, SOL_SOCKET, SO_KEEPALIVE, (char *)&SetKeepAlive, sizeof(SetKeepAlive)) != 0)
	{
		TRACE(_T("setsockopt() error\n"), WSAGetLastError());
	}

	// 设置超时详细信息
	unsigned long OptionKeepAlive = 1;

	tcp_keepalive klive;
	klive.onoff = 1; // 启用保活
	klive.keepalivetime = m_nKeepLiveTime;
	klive.keepaliveinterval = 1000 * 20; // 重试间隔为10秒 Resend if No-Reply
	WSAIoctl(pContext->m_Socket,
		SIO_KEEPALIVE_VALS,
		&klive,
...
bool CIOCPServer::InitializeIOCP(void)
{
	...
	// threads.
	m_hCompletionPort = CreateIoCompletionPort((HANDLE)Socket, NULL, 0, 0);
	...
	// We use two worker threads for each processor on the system--this is choosen as a good balance
...
void CIOCPServer::Send(ClientContext* pContext, LPBYTE lpData, UINT nSize)
{
	...
	try
	{
		...
		{
			// Compress data
			unsigned long destLen = (unsigned long)(1.001 * nSize + 12);
			LPBYTE pDest = new BYTE[destLen];
			//		int	nRet = compress2(pDest, &destLen, lpData, nSize,Z_BEST_COMPRESSION);
...
ClientContext*  CIOCPServer::AllocateContext()
{
	...
	if (pContext != NULL)
	{
		...
		pContext->m_hWriteComplete = CreateEvent(NULL, false, true, NULL);
