...
CString SEU_QQwry::GetStr(int ioffset)//根据位置读信息
{
	...
	MultiByteToWideChar(CP_ACP, 0, (char*)temp, 50, buf, 50);
	return buf;
}
#else
CString SEU_QQwry::GetStr(int ioffset)//根据位置读信息
...
void SEU_QQwry::SaveToFile(CString Name)
{
	CString str1, str2;
	DWORD	dwBytesWrite;
	HANDLE hFile = CreateFile(Name + _T(".txt"), GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
...
DWORD SEU_QQwry::IPtoDWORD(CString szIP)
{
	...
	{
		szTemp[j] = szIP.GetAt(i);
		j++;
		if (szIP.GetAt(i) == _T('.'))
		{
			char temp[4] = { 0 };
			wcstombs(temp, szTemp, 4);
			b[iXB] = atoi(temp);
